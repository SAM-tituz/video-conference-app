     ...
     <div className="min-h-screen p-4 flex flex-wrap gap-4">
      <div className="relative w-60 h-48">
        ...
        <div className="absolute bottom-2 left-2 flex gap-2">
          <button
            onClick={toggleMute}
            className={`p-2 rounded-full ${isMuted ? 'bg-red-500' : 'bg-gray-800/70'} hover:bg-gray-700/90`}
          >
            {isMuted ? (
          ...
          <button
            onClick={toggleVideo}
            className={`p-2 rounded-full ${!isVideoOn ? 'bg-red-500' : 'bg-gray-800/70'} hover:bg-gray-700/90`}
          >
            {isVideoOn ? (
              <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {/* {remoteStreams.map(({ id, stream }) => (
        <video
          ...
          ref={el => {
            ...
            else delete remoteVideosRef.current[id];
          }}
          autoPlay
          className="w-60 h-48 bg-black rounded-lg shadow-lg"
