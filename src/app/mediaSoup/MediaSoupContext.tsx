"use client";
import { createContext, useContext, useEffect, useRef, useState, SetStateAction, Dispatch, useCallback } from 'react';
...
type MediaSoupContextType = {
  ...
  participants: { id: string; name: string; isLocal?: boolean; isOrganizer?: boolean }[];
...
export const MediaSoupProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const client = mediaSoupClient(roomName, participantId, {
      ...
      onRemoteStream: (peerId, stream, kind) => {
        setRemoteStreams((prev) => {
          ...
          if (existingStream) {
            const oldTracks = existingStream.getTracks().filter((t) => t.kind !== kind);
        ...
        setRemoteStreams((prev) => {
          ...
          if (existingStream) {
            ...
            if (existingStream.getTracks().length === 0) {
              ...
              newStreams.set(peerId, new MediaStream(existingStream.getTracks()));
            }
          }
          return newStreams;
        });
      ...
      onMoveConfirmed: ({ newRoomName, rtpCapabilities }) => {
        ...
        clientRef.current?.reinitialize(rtpCapabilities, localStream || new MediaStream());
      },
    ...
    try {
      ...
      if (isVideoOn) {
        ...
        if (newTrack) {
          ...
          console.log('Resumed video track:', newTrack, 'New stream tracks:', newStream.getTracks());
        } else {
      ...
      alert("Failed to toggle video: " + (error instanceof Error ? error.message : String(error)));
    }
  }, [localStream]);
  ...
  const toggleMute = useCallback(async (): Promise<void> => {
    ...
    try {
      const isMuted = localStream.getAudioTracks().length === 0 || !localStream.getAudioTracks()[0]?.enabled;
      if (isMuted) {
      ...
      alert("Failed to toggle mute: " + (error instanceof Error ? error.message : String(error)));
    }
  }, [localStream]);
  ...
  const toggleScreenShare = useCallback(async (): Promise<void> => {
    ...
    try {
      const isScreenSharing = !!localStream.getVideoTracks().find(t => t.label.includes('screen'));
      if (isScreenSharing) {
      ...
      alert("Failed to toggle screen share: " + (error instanceof Error ? error.message : String(error)));
    }
  }, [localStream]);
...
export const useMediaSoup = () => {
  ...
  if (!context) throw new Error('useMediaSoup must be used within MediaSoupProvider');
  return context;
};